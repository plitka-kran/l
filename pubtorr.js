(function () {
    ...
    var parsersInfo = [
        ...
        { id: 'jac_black', name: 'jac.black', settings: { url: 'jac.black', key: '', parser_torrent_type: 'jackett' } },
    ...
    var cache = { data: {}, TTL: 10 * 60 * 1000, get: function get(key) { var cached = this.data[key]; if (cached && Date.now() < cached.expiresAt) return cached; return null; }, set: function set(key, value) { this.data[key] = _objectSpread2(_objectSpread2({}, value), {}, { expiresAt: Date.now() + this.TTL }); } };

    function getProtocol() { if (Lampa.Utils && typeof Lampa.Utils.protocol === 'function') return Lampa.Utils.protocol(); return location.protocol === 'https:' ? 'https://' : 'http://'; }
    ...
    function cacheKey(parserId, url) { return "".concat(parserId, "::").concat(url); }
    function statusFromXhr(xhr) { if (!xhr) return STATUS.networkError; if (xhr.status === 200) return STATUS.ok; if (xhr.status === 401) return STATUS.authError; return STATUS.networkError; }
    ...
    function checkAlive(parsers) {
        ...
        var requests = parsers.map(function (parser) {
            return new Promise(function (resolve) {
                ...
                if (!url) { results[parserId] = STATUS.unknown; resolve(); return; }
                var key = cacheKey(parserId, url);
                var cached = cache.get(key);
                if (cached) { results[parserId] = cached.status; resolve(); return; }
                $.ajax({
                    url: url,
                    method: 'GET',
                    timeout: 5000,
                    success: function success(response, textStatus, xhr) {
                        ...
                        if (xhr.status === 200 || xhr.status === 401) { cache.set(key, { status: status }); }
                        results[parserId] = status;
                        resolve();
                    },
                    error: function error(xhr) { results[parserId] = statusFromXhr(xhr); resolve(); }
                });
            });
        });
    ...
    // ===== Новая логика хранения =====
    var STORAGE_KEY_PRIMARY = 'lme_parser_primary';
    var STORAGE_KEY_SECONDARY = 'lme_parser_secondary';
    var NO_PARSER_ID = 'no_parser';

    function getSelectedParserId(type) {
        var key = type === 'secondary' ? STORAGE_KEY_SECONDARY : STORAGE_KEY_PRIMARY;
        return Lampa.Storage.get(key, NO_PARSER_ID);
    }

    function getParserById(parserId) {
        return parsersInfo.find(function (parser) { return parser.id === parserId; });
    }
    ...
    function applySelectedParser(parserId, type) {
        ...
        // Сохраняем в альтернативные ключи, если это дополнительная ссылка
        var urlKey = isSecondary ? 'jackett_url_alt' : 'jackett_url';
        var keyKey = isSecondary ? 'jackett_key_alt' : 'jackett_key';
        var typeKey = isSecondary ? 'parser_torrent_type_alt' : 'parser_torrent_type';

        Lampa.Storage.set(urlKey, settings.url);
        Lampa.Storage.set(keyKey, settings.key || '');
        Lampa.Storage.set(typeKey, parserType);

        // Для основной ссылки обновляем основные настройки
        if (!isSecondary) {
        ...
        var storageKey = isSecondary ? STORAGE_KEY_SECONDARY : STORAGE_KEY_PRIMARY;
        Lampa.Storage.set(storageKey, parserId);

        return true;
    }

    function applyStoredParserOnStart() {
        var primaryId = getSelectedParserId('primary');
        var secondaryId = getSelectedParserId('secondary');

        if (primaryId !== NO_PARSER_ID) {
            applySelectedParser(primaryId, 'primary');
            console.log('PubTorr', 'Applied primary parser:', primaryId);
        } else {
            console.log('PubTorr', 'No primary parser selected');
        }

        if (secondaryId !== NO_PARSER_ID) {
            applySelectedParser(secondaryId, 'secondary');
            console.log('PubTorr', 'Applied secondary parser:', secondaryId);
        } else {
            console.log('PubTorr', 'No secondary parser selected');
        }
    }
    ...
    function updateCurrentLabel(wrapper, selectedId, parsers) {
        var current = parsers.find(function (parser) { return parser.id === selectedId; });
    ...
    function updateSettingsSelectedLabel(selectedId, parsers, type) {
        var current = parsers.find(function (parser) { return parser.id === selectedId; });
        var label = current ? current.name : Lampa.Lang.translate('lme_parser_none');
        var typeText = type === 'primary' ? 'Основной' : 'Дополнительный';
        var text = "".concat(typeText, ": ").concat(label);
        $('.pubtorr-parser-selected-' + type).text(text);
    }

    function openParserModal(type) {
        var isSecondary = type === 'secondary';
        var title = isSecondary ? 'Дополнительный каталог' : 'Основной каталог';
        var parsers = [{ id: NO_PARSER_ID, name: Lampa.Lang.translate('lme_parser_none') }].concat(_toConsumableArray(parsersInfo));
        var selectedId = getSelectedParserId(type);
        ...
        parsers.forEach(function (parser) {
            ...
            item.on('hover:enter', function () {
                Lampa.Storage.set(isSecondary ? STORAGE_KEY_SECONDARY : STORAGE_KEY_PRIMARY, parser.id);
                applySelection(list, parser.id);
                updateCurrentLabel(modal, parser.id, parsers);
                updateSettingsSelectedLabel(parser.id, parsers, type);
        ...
        updateSettingsSelectedLabel(selectedId, parsers, type);
    ...
    function parserSetting() {
        ...
        Lampa.SettingsApi.addParam({
            component: 'parser',
            param: { name: 'lme_parser_primary', type: 'button' },
            field: {
                name: 'Основной каталог',
                description: "".concat(Lampa.Lang.translate('lme_parser_description'), " ").concat(parsersInfo.length, "<div class=\"pubtorr-parser-selected-primary\"></div>")
            },
            onChange: function onChange() { openParserModal('primary'); },
            onRender: function onRender(item) {
                var selectedId = getSelectedParserId('primary');
                var current = parsersInfo.find(function (parser) { return parser.id === selectedId; });
                var label = current ? current.name : Lampa.Lang.translate('lme_parser_none');
                item.find('.pubtorr-parser-selected-primary').text("Основной: ".concat(label));
                setTimeout(function () {
        ...
        Lampa.SettingsApi.addParam({
            component: 'parser',
            param: { name: 'lme_parser_secondary', type: 'button' },
            field: {
                name: 'Дополнительный каталог',
                description: "".concat(Lampa.Lang.translate('lme_parser_description'), " ").concat(parsersInfo.length, "<div class=\"pubtorr-parser-selected-secondary\"></div>")
            },
            onChange: function onChange() { openParserModal('secondary'); },
            onRender: function onRender(item) {
                var selectedId = getSelectedParserId('secondary');
                var current = parsersInfo.find(function (parser) { return parser.id === selectedId; });
                var label = current ? current.name : Lampa.Lang.translate('lme_parser_none');
                item.find('.pubtorr-parser-selected-secondary').text("Дополнительный: ".concat(label));
                setTimeout(function () {
                    var parserUse = $('div[data-name="parser_use"]').first();
                    if (parserUse.length) item.insertAfter(parserUse.next());
                    if (Lampa.Storage.field('parser_use')) item.show();
                    else item.hide();
                });
            }
        ...
        Lampa.SettingsApi.addParam({
            ...
            onRender: function onRender(item) {
                setTimeout(function () {
                    var manage = $('div[data-name="lme_parser_primary"]').first();
