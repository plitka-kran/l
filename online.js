// Online Mod (без прокси, с автоматической индикацией премиум-озвучки 44)
...
(function () {
    ...
    function rezka2(component, _object) {
        ...
        function fetchVoiceData(translator_id, season_id, callback) {
            ...
            dlog('fetchVoiceData() key=', key, 'ЗАПРОС...');
            var req = new Lampa.Reguest();
            req.timeout(10000);
            req.silent(url, function (json) {
                ...
                dlog('fetchVoiceData() key=', key, 'ОШИБКА/ТАЙМАУТ status=', a && a.status, 'err=', c, '-> закэшируется ПУСТОЙ результат навсегда до reset()!');
