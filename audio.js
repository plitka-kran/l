(function () {
    ...
    // ГЛОБАЛЬНЫЙ КЭШ ДЛЯ ХРАНЕНИЯ ХЭША ТОРРЕНТА И ПУТИ ПРИ СМЕНЕ СЕРИЙ
    var cached_torrent_hash = '';
    var last_loaded_id = null; 

    function reguest(params, callback) {
      // Проверяем, есть ли хэш. Если его нет ни в параметрах, ни в кэше — прекращаем работу
      var hash = params.torrent_hash || cached_torrent_hash;
      if (!hash) return;
      ...
      if (params.ffprobe && params.path.split('.').pop() !== 'mp4') {
        ...
        // Используем актуальный хэш
        var socket = new WebSocket('ws://' + connect_host + ':8080/?' + hash + '&index=' + params.id);
    ...
    function forceUpdateTracks(torrent_hash, episode_id, episode_path) {
      var video_data = {
        torrent_hash: torrent_hash || cached_torrent_hash,
        id: episode_id,
        path: episode_path || ''
      };
    ...
    // ПЕРЕХВАТ ИНИЦИАЛИЗАЦИИ И ОБНОВЛЕНИЯ ИНТЕРФЕЙСА ПЛЕЕРА
    ...
    Lampa.PlayerPanel.init = function () {
        ...
        if (current_player_data) {
            // Если в текущем запуске серии есть torrent_hash (первый запуск) — сохраняем/обновляем его в кэше
            if (current_player_data.torrent_hash) {
                cached_torrent_hash = current_player_data.torrent_hash;
            }

            // Получаем данные текущей серии из плейлиста
            ...
            // Если у нас есть хэш (из кэша или прямой) и мы еще не обрабатывали этот ID серии
            if (cached_torrent_hash && last_loaded_id !== real_id) {
                last_loaded_id = real_id;
                
                // Даем Tizen 400мс на подгрузку нативных дорожек, затем заменяем их
                setTimeout(function() {
                    forceUpdateTracks(
                        cached_torrent_hash, 
                        real_id, 
    ...
    // При полном выходе из плеера сбрасываем кэш, чтобы не смешивать релизы
    Lampa.Player.listener.follow('destroy', function() {
        last_loaded_id = null;
        cached_torrent_hash = '';
    });

    // --- Логика для вкладки "Инфо" в списке файлов торрента ---
