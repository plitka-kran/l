(function () {
    ...
    function forceUpdateTracks(data) {
      log('🔄 Force update tracks called with data:', data);
      ...
      log('🔄 Video audio tracks:', video_tracks.length, video_tracks);
      ...
      parse_tracks.forEach(function (track, index) {
        ...
        log('🔄 Processing track:', index, 'track index:', track.index, 'minus:', minus, 'orig:', orig);
      ...
      if (parse_tracks.length) {
        log('🔄 Setting tracks:', new_tracks);
        Lampa.PlayerPanel.setTracks(new_tracks);
        log('✅ Tracks updated successfully!');
      } else {
    ...
    function forceUpdateSubs(data) {
      ...
      parse_subs.forEach(function (track, index) {
        ...
        log('🔄 Processing sub:', index, 'track index:', track.index, 'minus:', minus, 'orig:', orig);
      ...
      if (parse_subs.length) {
        log('🔄 Setting subs:', new_subs);
        Lampa.PlayerPanel.setSubs(new_subs);
        log('✅ Subs updated successfully!');
      } else {
    ...
    function subscribeTracks(data) {
      log('🎬 subscribeTracks called with data:', data);
      ...
      function listenTimeUpdate() {
        ...
        if (video && video.currentTime < 0.5) {
          ...
          if (inited_parse) {
            ...
            warnLog('⏱️ No parsed data available');
          }
        }
      }
    ...
    // Добавляем полезные функции в консоль
    window.debugTracks = {
      getState: function() {
        var video = Lampa.PlayerVideo.video();
        return {
    ...
    log('🚀 Tracks script initialized');
    log('💡 Use debugTracks.forceUpdate() to manually update');
    log('💡 Use debugTracks.getState() to check current state');

    // Слушаем событие смены серии
    ...
    log('✅ Tracks script fully loaded');
})();
